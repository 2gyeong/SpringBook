...
public class DispatcherServlet extends HttpServlet {
	...
	// D. 백엔드의 로직을 모두 처리 후 View 페이지로 이동
		if(userD != null) { // 클라이언트에게 전송한 ID와 Pass가 DB의 값과 일치할때
			...
			// dto 의 setter 메소드 호출 시 클라이언트에게 넘어오는 변수를 할당.
			dto.setTitle(title);
			dto.setWriter(writer);
			dto.setContent(content);
			
			dao.insertBoard(dto); // DB에 Insert가 완료됨. ( Insert는 void임)
									// Insert / Update / delete 는 리턴 값이 없음.
		...
		System.out.println("seq 변수 값 : "+seq);
		...
		// DB의 값이 저장된 DTO(board)를 session 변수에 할당해서 View 페이지로 전달
		HttpSession session = request.getSession();
...
//			System.out.println(content);
			...
			// 1. 클라이언트에서 넘긴 seq 를 받아서 변수에 저장
			String seq = request.getParameter("seq");
			
			// 2. DTO, DAO에 로직 처리 (백엔드의 로직 처리 )
			BoardDTO dto = new BoardDTO();
			BoardDAO dao = new BoardDAO();
